...
- We were able to utilize our existing [get_time_refs](#get_time_refs) function and create a simplified version for the categorical variables.

- This is a helper function for the [function that actually performs the conversion](#convert_cat_code_to_description), described below
...
Here we handle the numeric variables. PWGTP and at least one other valid numeric variable must be selected. We have our lists of valid inputs in the [utility functions](#utilities) section for reference. We will check against these to ensure each variable in the input is valid.
...
- The data frame that came out of the JSON-to-data-frame helper had all the data in columns, but everything was character, so a [final helper function](#process_census_data) takes care of cleaning the data
...
Generic functions were created to automatically summarize and plot certain returned data. These are the two additional interfaces that we built for the user. The expect a tibble from a successful API call. 
...
For our investigative demonstration, we have chosen to examine how arrival time to work (JWAP) is related to household language (HHL) in the state of North Carolina for the year 2015.
...
To showcase our multi-year function, we will view the monthly gas cost (GASP) for the Northeast region, for years 2016 through 2018, by whether they have given birth within the last 12 months (FER).

***This is taking way too long, not executing this code block for now***

```{r}
#| eval = FALSE

test_multi_year <- query_census_multiple_years(2016:2018, 
                                               c("PWGTP", "GASP"), 
                                               "FER", 
                                               geography = "Region", 
                                               subset = "Northeast")
