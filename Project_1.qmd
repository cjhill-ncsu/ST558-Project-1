...
In the construction of the URL, this function will take in the combination of Geography and Subset to return that particular subset's numeric code to be used in the URL itself. If the subset is NULL, we simply return an asterisk to be used with any geography the user might provide. Otherwise, we need to retrieve the appropriate subset pertaining to the given geography.
...
- Another fantastic example of function reuse. Initially for get_state_code, the codes and their corresponding states were hardcoded as a named vector and contained within. This was refactored however to use the previously defined [get_cat_refs()](#get_cat_refs). 
